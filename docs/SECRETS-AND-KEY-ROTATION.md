...
| `ALLOWED_CORS_ORIGINS` | Comma-separated origins for telemetry `POST` CORS | Not a secret; include production + experimental preview URLs (see `apps/funnel-dashboard/.env.example`). Redeploy **funnel-dashboard** (`solar-form-52ub`) after changes. |
